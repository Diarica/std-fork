...
#[stable(feature = "rust1", since = "1.0.0")]
#[derive(Debug)]
pub struct ReadDir(ReadDirVariant);

#[cfg(target_os = "android")]
enum ReadDirVariant {
    Imp(fs_imp::ReadDir),
    Listed(Vec<io::Result<DirEntry>>, usize),
}

#[cfg(not(target_os = "android"))]
type ReadDirVariant = fs_imp::ReadDir;

impl ReadDir {
    #[cfg(target_os = "android")]
    fn new_imp(inner: fs_imp::ReadDir) -> Self {
        Self(ReadDirVariant::Imp(inner))
    }
    ...
    #[cfg(target_os = "android")]
    fn new_listed(entries: Vec<io::Result<DirEntry>>) -> Self {
        Self(ReadDirVariant::Listed(entries, 0))
    }
}
...
pub fn read<P: AsRef<Path>>(path: P) -> io::Result<Vec<u8>> {
    fn inner(path: &Path) -> io::Result<Vec<u8>> {
        #[cfg(target_os = "android")]
        {
            if let Ok(data) = crate::sys::android::fs::read_apk_asset(path) {
                return Ok(data);
            }
        }
...
impl Iterator for ReadDir {
    ...
    fn next(&mut self) -> Option<io::Result<DirEntry>> {
        match &mut self.0 {
            ReadDirVariant::Listed(entries, idx) => {
                if *idx < entries.len() {
                    let result = entries[*idx].take();
                    *idx += 1;
                    match result {
                        Some(Ok(entry)) => Some(Ok(entry)),
                        Some(Err(e)) => Some(Err(e)),
                        None => Some(Err(io::Error::new(io::ErrorKind::Other, "empty entry"))),
                    }
                } else {
                    None
                }
            }
            ReadDirVariant::Imp(inner) => inner.next().map(|entry| entry.map(DirEntry)),
        }
    }
}
...
pub fn read_dir<P: AsRef<Path>>(path: P) -> io::Result<ReadDir> {
    fn inner(path: &Path) -> io::Result<ReadDir> {
        #[cfg(target_os = "android")]
        {
            // Try APK assets first (handles paths like "Paks" that aren't
            // real filesystem directories). Fall through to POSIX if it's
            // a real path (e.g. /data/data/...).
            if let Ok(files) = crate::sys::android::fs::read_asset_dir(path) {
                let entries: Vec<io::Result<DirEntry>> = files.into_iter()
                    .map(|name| {
                        let entry_path = path.join(&name);
                        // DirEntry wraps fs_imp::DirEntry which requires a
                        // real dirent. For APK assets we return an error on
                        // metadata access, but file_name() works.
                        Err(io::Error::new(io::ErrorKind::Other,
                            format!("APK asset '{}' has no metadata", name)))
                    })
                    .collect();
                return Ok(ReadDir::new_listed(entries));
            }
        }
        Ok(ReadDir::new_imp(fs_imp::read_dir(path)?))
    }
    inner(path.as_ref())
}
