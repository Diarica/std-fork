...
fn main() {
    ...
    // Link Android NDK for AAssetManager support.
...
fn compile_marl_if_supported() {
    ...
    let marl_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("vendor").join("marl");
    if !marl_root.join("include").join("marl").join("scheduler.h").exists() {
        return;
    }
    let c_api_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("src").join("c_api");
    let c_api_cpp = c_api_dir.join("marl_c_api.cpp");
    if !c_api_cpp.exists() {
        return;
    }
    let mut build = cc::Build::new();
    build.cpp(true);
    build.include(marl_root.join("include"));
    build.include(&c_api_dir);
    build.warnings(false);
    for f in &["src/scheduler.cpp", "src/thread.cpp", "src/debug.cpp",
               "src/memory.cpp", "src/trace.cpp"] {
        build.file(marl_root.join(f));
    }
    build.file(&c_api_cpp);
